...
public class lb_BirdController : MonoBehaviour {
    ...
    if(activeBirds < idealNumberOfBirds && AreThereActiveTargets()){
        ...
        // Changed from .05 to .15 - spawn more frequently (every ~7 seconds instead of 20)
        SpawnBird();
    }
	...
	Vector3 FindPositionOffCamera()
{
    ...
    // Spawn birds AHEAD and to the SIDES of the camera view
    float dist = Random.Range(15, 40); // Distance ahead
    
    // Get camera forward direction
    Vector3 ray = currentCamera.transform.forward;
    
    // Add random offset to sides and up/down (keeps birds in peripheral vision)
    ray += new Vector3(
        Random.Range(-0.8f, 0.8f),  // Left/Right spread
        Random.Range(-0.3f, 0.5f),  // Up/Down (bias upward)
        Random.Range(-0.2f, 0.2f)   // Slight depth variation
    );
    ...
    // Find a spawn point that doesn't hit anything
    while(Physics.Raycast(currentCamera.transform.position, ray, out hit, dist))
    {
        dist = Random.Range(15, 40);
        ray = currentCamera.transform.forward;
        ray += new Vector3(
